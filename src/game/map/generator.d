...
void GenerateMap(Map map)
{
	...
	foreach(j; 0 .. 4)
	{
		...
		foreach (z; 0 .. 16)
		{
		int noiseVal = noise((16*i + x) / 64.0, (16*j + z) / 64.0, 7) / 4;

		for(int y = 0; y < 16 && y < noiseVal; y++)
		{
			if( y < noiseVal - 3)
			{
				chunk0.hexes[x][y][z] = 1;
			}
			else if(y < noiseVal - 1)
			{
				chunk0.hexes[x][y][z] = 2;
			}
			else
			{
				chunk0.hexes[x][y][z] = 3;
			}
		}
		for(int y = 16; y < 32 && y < noiseVal; y++)
		{
			if( y < noiseVal - 3)
			{
				chunk1.hexes[x][y%16][z] = 1;
			}
			else if(y < noiseVal - 1)
			{
				chunk1.hexes[x][y%16][z] = 2;
			}
			else
			{
				chunk1.hexes[x][y%16][z] = 3;
			}
		}
		for(int y = 32; y < 48 && y < noiseVal; y++)
		{
			if( y < noiseVal - 3)
			{
				chunk2.hexes[x][y%16][z] = 1;
			}
			else if(y < noiseVal - 1)
			{
				chunk2.hexes[x][y%16][z] = 2;
			}
			else
			{
				chunk2.hexes[x][y%16][z] = 3;
			}
		}
		for(int y = 48; y < 64 && y < noiseVal; y++)
		{
			if( y < noiseVal - 3)
			{
				chunk3.hexes[x][y%16][z] = 1;
			}
			else if(y < noiseVal - 1)
			{
				chunk3.hexes[x][y%16][z] = 2;
			}
			else
			{
				chunk3.hexes[x][y%16][z] = 3;
			}
		}
		...
		map.setChunk(chunk0, i,0,j);
		map.setChunk(chunk1, i,1,j);
		map.setChunk(chunk2, i,2,j);
		map.setChunk(chunk3, i,3,j);
	}
}

public int noise(double x,double y,int nbOctave)
{
   int result=0;      
   int frequence256=256; 
   int sx=cast(int)((x)*frequence256); 
   int sy=cast(int)((y)*frequence256); 
   int octave=nbOctave;   
   while(octave!=0) 
   {
      int bX=sx&0xFF;
       ...
       *
       * the last part of the PRN (alt1&0xFF0000)>>8 is used as an 
       * offset to correct one of the gradiant problem wich is zero on cell edge
       *
